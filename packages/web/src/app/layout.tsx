...
import { Anchor, LayoutDashboard, Settings, Globe, Menu, X } from 'lucide-react';
              ...
              <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16">
                  ...
                  <div className="flex items-center space-x-2 sm:space-x-3">
                    <div className="p-2 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 shadow-lg shadow-blue-500/50">
                      <Anchor className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
                    </div>
                  ...
                  <div className="flex md:hidden items-center">
                    <button
                      ...
                      className="p-2 rounded-lg bg-slate-100/50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300"
                    >
                ...
                {mobileMenuOpen && (
                  <div className="md:hidden pb-4 space-y-2">
                    <MobileNavLink href="/" icon={LayoutDashboard} onClick={() => setMobileMenuOpen(false)}>
                      Dashboard
                    </MobileNavLink>
                    <MobileNavLink href="/urls" icon={Globe} onClick={() => setMobileMenuOpen(false)}>
                      URLs
                    </MobileNavLink>
...
function MobileNavLink({ href, icon: Icon, children, onClick }: { href: string; icon: any; children: React.ReactNode; onClick: () => void }) {
  return (
    <Link
      href={href}
      onClick={onClick}
      className="flex items-center space-x-3 px-4 py-3 rounded-xl bg-gradient-to-br from-slate-100/50 to-slate-200/50 dark:from-slate-800/50 dark:to-slate-700/50 backdrop-blur-sm border border-slate-200/50 dark:border-slate-700/50 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-all"
    >
